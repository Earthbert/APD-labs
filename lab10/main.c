...
#define MPI_TAG_NUMBER_OF_NODES 6
...
int *get_dst(int rank, int numProcs, int leader) {
	...
	*/
	if (rank != leader) {
		...
		MPI_Recv(vRecv, numProcs, MPI_INT, v[rank], MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		for (size_t i = 0; i < numProcs; i++) {
			if (v[i] == -1 && vRecv[i] != -1) {
				v[i] = vRecv[i];
			}
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (int i = 0; i < num_neigh; i++) {
			...
			MPI_Recv(&q, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			if (q > leader) {
				leader = q;
			}
		}
...
int get_number_of_nodes(int rank, int leader) {
	...
	for (int k = 0; k < CONVERGENCE_COEF; k++) {
		...
		*/
		for (size_t i = 0; i < num_neigh; i++) {
			MPI_Send(&val, 1, MPI_DOUBLE, neigh[i], MPI_TAG_NUMBER_OF_NODES, MPI_COMM_WORLD);
			MPI_Recv(&recvd, 1, MPI_DOUBLE, neigh[i], MPI_TAG_NUMBER_OF_NODES, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			val = (val + recvd) / 2.0;
		}
	}
...
int **get_topology(int rank, int nProcesses, int *parents, int leader) {
	int *topology = malloc(sizeof(int) * nProcesses * nProcesses);
	int *vTopology = malloc(sizeof(int) * nProcesses * nProcesses);

	for (size_t i = 0; i < nProcesses; i++) {
		for (size_t j = 0; j < nProcesses; j++) {
			topology[i * nProcesses + j] = 0;
		}
	}


	for (size_t i = 0; i < num_neigh; i++) {
		topology[rank * nProcesses + neigh[i]] = 1;
	}
	...
	for (size_t i = 0; i < num_neigh; i++) {
		if (neigh[i] != parents[rank]) {
			MPI_Recv(vTopology, nProcesses * nProcesses, MPI_INT, neigh[i], MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			for (size_t j = 0; j < nProcesses; j++) {
				for (size_t k = 0; k < nProcesses; k++) {
					if (vTopology[j * nProcesses + k] == 1) {
						topology[j * nProcesses + k] = 1;
					}
				}
			}
		}
	...
	if (rank != leader)
		MPI_Send(topology, nProcesses * nProcesses, MPI_INT, parents[rank], 0, MPI_COMM_WORLD);

	/* TODO4: Daca nu suntem liderul, asteptam topologia completa de la parinte  */
	if (rank != leader) {
		MPI_Recv(vTopology, nProcesses * nProcesses, MPI_INT, parents[rank], MPI_ANY_TAG, MPI_COMM_WORLD, &status);
		for (size_t j = 0; j < nProcesses; j++) {
			for (size_t k = 0; k < nProcesses; k++) {
				if (vTopology[j * nProcesses + k] == 1) {
					topology[j * nProcesses + k] = 1;
	...
	for (size_t i = 0; i < num_neigh; i++) {
		if (neigh[i] != parents[rank]) {
			MPI_Send(topology, nProcesses * nProcesses, MPI_INT, neigh[i], 0, MPI_COMM_WORLD);
		}
	}

	int **return_topology = malloc(sizeof(int *) * nProcesses);
	for (size_t i = 0; i < nProcesses; i++) {
		return_topology[i] = malloc(sizeof(int) * nProcesses);
	}

	for (size_t i = 0; i < nProcesses; i++) {
		for (size_t j = 0; j < nProcesses; j++) {
			return_topology[i][j] = topology[i * nProcesses + j];
		}
	}
