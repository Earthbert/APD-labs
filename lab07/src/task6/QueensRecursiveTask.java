...
public class QueensRecursiveTask extends RecursiveTask<Void> {

	private final int[] graph;
	private final int step;

	public QueensRecursiveTask(int[] graph, int step) {
		this.graph = graph;
		this.step = step;
	}
	...
	protected Void compute() {
		...
		List<QueensRecursiveTask> tasks = new ArrayList<>();
		...
		for (int i = 0; i < Main.N; ++i) {
			...
			if (check(newGraph, step)) {
				QueensRecursiveTask task = new QueensRecursiveTask(newGraph, step + 1);
				tasks.add(task);
				task.fork();
			}
		}

		for (QueensRecursiveTask task : tasks) {
			task.join();
		}
